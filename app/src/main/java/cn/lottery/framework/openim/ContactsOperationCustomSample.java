 ...
 * @author shuheng
 */
public class ContactsOperationCustomSample extends IMContactsOperation implements SuccessListener<JSONObject>, ErrorListener {
    ...
    public boolean onListItemClick(Fragment fragment, IYWContact contact) {
        if (contact.getAppKey().equals(YWConstants.YWSDKAppKey)) {
            ...
            Intent intent = OpenIMLoginHelper.getInstance().getIMKit().getChattingActivityIntent(eServiceContact);
            if(intent!=null)
                fragment.getActivity().startActivity(intent);
        } else {
            Intent intent =OpenIMLoginHelper.getInstance().getIMKit().getChattingActivityIntent(contact.getUserId(), contact.getAppKey());
            if(intent!=null)  fragment.getActivity().startActivity(intent);
        }
        return true;
    }
    ...
    public boolean onListItemLongClick(Fragment fragment, final IYWContact contact) {
        ...
        final IYWContactService contactService = OpenIMLoginHelper.getInstance().getIMKit().getContactService();
    ...
    */
    ...
    public boolean onListItemLongClick(Fragment fragment, final IYWContact contact) {
        ...
        items[0] = "删除好友";
        //RelativeLayout customView = (RelativeLayout) mContext.getLayoutInflater().inflate(R.layout.openim_custom_contacts_title_bar, new RelativeLayout(mContext), false);
        //customView.setBackgroundColor(Color.parseColor("#f4bc1a"));
        //TextView title = (TextView) customView.findViewById(R.id.title_txt);
        //title.setText(contact.getShowName()); //联系人
                         ...
                         if (items[which].equals("删除好友")) {
                            contactService.delContact(contact.getUserId(), contact.getAppKey(), new IWxCallback() {
                                ...
                                public void onSuccess(Object... result) {
                                    ...
                                    YWLog.i(TAG, "删除好友成功,  id = " + iywContact.getUserId() + ", appkey = " + iywContact.getAppKey());
                                    //IMNotificationUtils.getInstance().showToast(mContext, "删除好友成功,  id = " + iywContact.getUserId() + ", appkey = " + iywContact.getAppKey());
                                    IMNotificationUtils.getInstance().showToast(mContext, "删除好友成功");
                                    operateContactCallBack(Config.IMUserId,iywContact.getUserId(),"3");
                                }
