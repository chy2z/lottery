...
import cn.lottery.R;
import cn.lottery.app.activity.openim.contact.OpenimContactSettingActivity;
...
/**
 * 聊天界面自带提供两种主题的自定义供用户方便的使用，用户可以通过{@link OpenimCustomSampleHelper｝中 实现 AdviceBinder.bindAdvice(PointCutEnum.CHATTING_FRAGMENT_UI_POINTCUT, ChattingUICustomSample.class);
...
public class ChattingUICustomSample extends IMChattingPageUI {
    ...
    public int getMsgBackgroundResId(YWConversation conversation, YWMessage message, boolean self) {
        ...
        if (msgType == YWMessage.SUB_MSG_TYPE.IM_TEXT || msgType == YWMessage.SUB_MSG_TYPE.IM_AUDIO){
            if (self){
                return R.drawable.openim_talk_pop_r_bg;
            } else {
                return R.drawable.openim_talk_pop_l_bg;
            }
        } else if (msgType == YWMessage.SUB_MSG_TYPE.IM_IMAGE){
            if (self){
                return R.drawable.openim_talk_pic_pop_r_bg;
            } else {
                return R.drawable.openim_talk_pic_pop_l_bg;
            }
    ...
    public Bitmap processBitmapOfLeftImageMsg(Bitmap input) {
        ...
        Bitmap distBitmap = YWIMImageUtils.getFromCacheOrDecodeResource(R.drawable.openim_left_bubble);
    ...
    public Bitmap processBitmapOfRightImageMsg(Bitmap input) {
        Bitmap output = Bitmap.createBitmap(input.getWidth(),
                input.getHeight(), Bitmap.Config.ARGB_8888);
        //为提高性能，对取得的resource图片做缓存
        Bitmap distBitmap = YWIMImageUtils.getFromCacheOrDecodeResource(R.drawable.openim_right_bubble);
        NinePatch np = new NinePatch(distBitmap, distBitmap.getNinePatchChunk(), null);
        Canvas canvas = new Canvas(output);
        final Paint paint = new Paint();
        final Rect rectSrc = new Rect(0, 0, input.getWidth(), input.getHeight());
...
//        }
    ...
    public View getCustomTitleView(final Fragment fragment,
        ...
        View view = inflater.inflate(R.layout.openim_custom_chatting_title, new RelativeLayout(context), false);
        view.setBackgroundColor(context.getResources().getColor(R.color.title_bg)); //#00b4ff
        ...
        backView.setTextColor(Color.parseColor("#000000"));
        backView.setTextSize(15);
        //backView.setCompoundDrawablesWithIntrinsicBounds(R.drawable.openim_common_back_btn_white, 0, 0, 0);
        backView.setCompoundDrawablesWithIntrinsicBounds(R.drawable.openim_common_back_btn_black, 0, 0, 0);
        ...
        if (conversation.getConversationType() == YWConversationType.Tribe) {
            btn.setImageResource(R.drawable.aliwx_tribe_info_icon_black);
            btn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    //Toast.makeText(context, "YWConversationType.Tribe", Toast.LENGTH_SHORT).show();
                    String conversationId = conversation.getConversationId();
                    long tribeId = Long.parseLong(conversationId.substring(5));
                    Intent intent = new Intent(fragment.getActivity(), OpenimTribeInfoActivity.class);
            ...
            btn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    //Toast.makeText(context, "YWConversationType.P2P", Toast.LENGTH_SHORT).show();
                    ...
                    Intent intent = OpenimContactSettingActivity.getContactSettingActivityIntent(context, appKey, userId);
            ...
            //Toast.makeText(context, conversation.getConversationId(), Toast.LENGTH_SHORT).show();
            btn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Toast.makeText(context, "YWConversationType.SHOP", Toast.LENGTH_SHORT).show();
                }
            });
            btn.setVisibility(View.VISIBLE);
        }
...
//                + "/alibaba/WXOPENI/云旺相册";
    ...
    private void receiveTribeMsg(YWTribe tribe, final View view) {
        OpenIMLoginHelper.getInstance().getIMKit().getIMCore().getTribeService().unblockTribe(tribe, new IWxCallback() {
            @Override
