...
import cn.lottery.R;
import cn.lottery.app.activity.trace.TraceAddContactOrTribeActivity;
...
public class ConversationListUICustomSample extends IMConversationListUI {
    ...
    public View getCustomConversationListTitle(final Fragment fragment,
        ...
        RelativeLayout customView = (RelativeLayout) inflater
                .inflate(R.layout.openim_custom_conversation_title_bar, new RelativeLayout(context),false);
        customView.setBackgroundColor(Color.parseColor("#f4bc1a"));
        TextView title = (TextView) customView.findViewById(R.id.title_txt);
        final YWIMKit mIMKit = OpenIMLoginHelper.getInstance().getIMKit();

        //title.setText(mIMKit.getIMCore().getShowName()); //显示昵称

        title.setText("聊天列表");
        title.setTextColor(Color.BLACK);

        final String loginUserId = OpenIMLoginHelper.getInstance().getIMKit().getIMCore().getLoginUserId();
        final String appKey = OpenIMLoginHelper.getInstance().getIMKit().getIMCore().getAppKey();
        ...
        */
        ...
        rightButton.setText("添加好友");
        rightButton.setTextColor(Color.BLACK);
        rightButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                //mIMKit.getConversationService().markAllReaded(); //消息标记已读
                Intent intent = new Intent().setClass(context, TraceAddContactOrTribeActivity.class);
    ...
    public View getCustomView(Context context, YWConversation conversation, View convertView, ViewGroup parent) {
        ...
        if(convertView==null){
            ...
            convertView = inflater.inflate(R.layout.openim_conversation_custom_view_item,
                    parent,
                    false);
    ...
    public View getCustomItemView(Fragment fragment, YWConversation conversation, View convertView, int viewType, YWContactHeadLoadHelper headLoadHelper, ViewGroup parent) {
        if (viewType == viewTypeArray[0]){
            ...
            if (convertView == null){
                ...
                convertView = inflater.inflate(R.layout.openim_custom_conversation_item_1, parent, false);
            ...
            if (conversation.getConversationBody() instanceof YWCustomConversationBody) {
                ...
                if(conversationId.equals(Constant.SYSTEM_FRIEND_REQ_CONVERSATION)){
...
//                convertView = inflater.inflate(R.layout.openim_custom_conversation_item_2, parent, false);
//                holder.head = (ImageView) convertView.findViewById(R.id.head);
//                holder.unread = (TextView) convertView.findViewById(R.id.unread);
//                holder.name = (TextView) convertView.findViewById(R.id.name);
