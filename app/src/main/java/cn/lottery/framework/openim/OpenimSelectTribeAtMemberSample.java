 ...
 */
public class OpenimSelectTribeAtMemberSample extends IMSelectTribeAtMemeberPageUI {

    public OpenimSelectTribeAtMemberSample(Pointcut pointcut) {
        super(pointcut);
    }
    ...
    public View getCustomTitle(final Activity activity, Context context, LayoutInflater inflater) {
        View view = LayoutInflater.from(activity).inflate(R.layout.openim_custom_select_at_member_title, new RelativeLayout(context));
        //TODO 这里设置背景最好用color的方式来设置,因为界面中的一些字体颜色需要跟titleBar主题颜色一致
        view.setBackgroundColor(activity.getResources().getColor(R.color.title_bg));
