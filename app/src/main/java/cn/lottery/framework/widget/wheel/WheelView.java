...
package cn.lottery.framework.widget.wheel;
...
public class WheelView extends View {
	...
	private void initResourcesIfNecessary() {
		...
		setBackgroundResource(R.drawable.f_wheel_bg);
	}
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			float textWidth = (float)Math.ceil(Layout.getDesiredWidth("0",itemsPaint)); //FloatMath.ceil(Layout.getDesiredWidth("0",itemsPaint));
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int)  Math.ceil(Layout.getDesiredWidth(label,valuePaint)); //FloatMath.ceil(Layout.getDesiredWidth(label,valuePaint));
		}
