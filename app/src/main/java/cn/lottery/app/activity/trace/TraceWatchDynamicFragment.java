 ...
 */
public class TraceWatchDynamicFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {

    private String TAG = TraceWatchDynamicFragment.class.getSimpleName();
     ...
     * 获取关注动态信息
     */
    private void requestWatchDynamic(){
        HashMap<String, String> params = new HashMap<String, String>();
        params.put("userId",Config.customerID);
        params.put("user_token",Config.userToken);
        showLoadingDialog();
        post(Config.URL_PREFIX + RequestUrl.getWatchDynamic, params, null,"requestWatchDynamic");
    }
    ...
    protected void handSuccess(JSONObject obj, Object tag) {
        if (tag.equals("requestWatchDynamic")) {
            closeDialog();
            processWatchDynamic(obj);
        }
     ...
     * @param json
     */
    private void processWatchDynamic(JSONObject json) {
        try {
            ...
            for (int i = 0; i < resultList.length(); i++) {
                ...
                hashMap.put("watchStatus","特别关注"); //没有watchStatus属性
            ...
            lvAdapter = new HomePageDynamicListViewAdapter(getContext(),HomePageDynamicListViewAdapter.SpecialWatch_Type, dataLV,app.getImageLoader());
            ...
            lvAdapter.setCommentListener(new HomePageDynamicListViewAdapter.CommentListener() {
                ...
                @Override
                public void addWatch(int position,Button watch,final String dynamicId, final String userid, final String user_token){

                    //toast("dynamicId:"+dynamicId+",userId:"+userid+",user_token:"+user_token);

                    mposition=position;

                    final AlertDialog myDialog = new AlertDialog.Builder(getContext(),AlertDialog.THEME_HOLO_LIGHT).create();
                    myDialog.show();
    ...
    private void processAddWatch(JSONObject json){
        try {
            ...
            JSONObject msg = json.getJSONObject("msg");
            if(msg.getBoolean("success")) {
                lvAdapter.getData().remove(mposition);
                lvAdapter.notifyDataSetChanged();
                hideInputMethod(getActivity());
            }
            toast(msg.getString("info"));
