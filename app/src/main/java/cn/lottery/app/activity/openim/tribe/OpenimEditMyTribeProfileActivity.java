 ...
 * 群名片
 */
public class OpenimEditMyTribeProfileActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.openim_activity_edit_my_tribe_profile);
    ...
    private void uploadModifiedUserNick(final String userNick){
        mTribeService.modifyTribeUserNick(tribeId, mIMKit.getIMCore().getAppKey(), userId, userNick, new IWxCallback() {
            ...
            public void onSuccess(Object... result) {
                IMNotificationUtils.getInstance().showToast("修改成功", OpenimEditMyTribeProfileActivity.this);
                Intent intent = new Intent();
                intent.putExtra(TribeConstants.TRIBE_NICK, userNick);
                setResult(Activity.RESULT_OK, intent);
                OpenimEditMyTribeProfileActivity.this.finish();
            }

            @Override
            public void onError(int code, String info) {
                IMNotificationUtils.getInstance().showToast("修改失败",OpenimEditMyTribeProfileActivity.this);
            }
