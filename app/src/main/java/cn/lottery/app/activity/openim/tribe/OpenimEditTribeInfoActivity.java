 ...
 * 创建群和编辑群信息
 */
public class OpenimEditTribeInfoActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.openim_activity_edit_tribe_info);
        init();
    }
    ...
    private void updateTribeInfo() {
        String name = mTribeName.getText().toString();  //群名称
        String notice = mTribeNotice.getText().toString(); //群公告
        ...
        if(name.equals(oldTribeName) && !notice.equals(oldTribeNotice)) { //只修改群公告
            mTribeService.modifyTribeInfo(callback, mTribeId, null, notice);
        } else if(!name.equals(oldTribeName) && notice.equals(oldTribeNotice)) { //只修改群名称
    ...
    private void createTribe(final YWTribeType type) {
        ...
        mTribeService.createTribe(new IWxCallback() {
            ...
            public void onSuccess(Object... result) {
                if (result != null && result.length > 0) {
                    ...
                    if (type.equals(YWTribeType.CHATTING_TRIBE)) {
                        IMNotificationUtils.getInstance().showToast(OpenimEditTribeInfoActivity.this, "创建群组成功！");
                    } else {
                        IMNotificationUtils.getInstance().showToast(OpenimEditTribeInfoActivity.this, "创建讨论组成功！");
                    }
                    //进入群信息界面
                    Intent intent = new Intent(OpenimEditTribeInfoActivity.this, OpenimTribeInfoActivity.class);
            ...
            public void onError(int code, String info) {
                IMNotificationUtils.getInstance().showToast(OpenimEditTribeInfoActivity.this, "创建讨论组失败，code = " + code + ", info = " + info);
            }
    ...
    class ModifyTribeInfoCallback implements IWxCallback {
        @Override
        public void onSuccess(Object... result) {
            IMNotificationUtils.getInstance().showToast(OpenimEditTribeInfoActivity.this, "修改群信息成功！");
            finish();
        }

        @Override
        public void onError(int code, String info) {
            IMNotificationUtils.getInstance().showToast(OpenimEditTribeInfoActivity.this, "修改群信息失败，code = " + code + ", info = " + info);
        }
